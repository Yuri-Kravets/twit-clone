...
export default function Home() {
    ...
    const handleLike = (e: React.MouseEvent, id:  string | number) => {
        ...
        setTweets(prevTweets =>
            prevTweets.map(tweet => (tweet._id ?? tweet.id)  === id ? {...tweet, reactions: {...tweet.reactions, likes: tweet.reactions.likes + 1}} : tweet))
    }
    const handleDislike = (e: React.MouseEvent, id: string | number) =>{
        e.stopPropagation();
        e.preventDefault();
        setTweets((prev) =>
            prev.map((tweet) => (tweet._id ?? tweet.id)=== id ?
                {...tweet,
                reactions: {
                    ...tweet.reactions,
            ...
            <section className="space-y-4">
                ...
                <div className="space-y-4">
                    {tweets.map((tweet) => (
                        ...
                        <Link key={tweet._id || tweet.id} href={`/tweet/${tweet._id || tweet.id}`}>
                            <div className="border p-4 rounded hover:bg-gray-50 cursor-pointer transition">
                                ...
                                <div
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        <button onClick={(e) =>
                                            handleLike(e, tweet._id || tweet.id)}
